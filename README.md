...
| Restrict to POA cells only | *(Atlas restriction)* When checked, the **entire** pipeline (cluster means, marker genes, AUCell, empirical null, Cre-driver baseline filter, correlation, Fisher, NNLS, GSEA, the per-cell AUCell UMAP) runs over preoptic-area cells only — directly comparable to HypoMap Table S1. When off, the full hypothalamic atlas is used | OFF |
| POA keywords (comma-separated) | Case-insensitive substring match against `Region_summarized` defining "POA" (e.g. add `paraventricular`, `anteroventral`) | `preoptic` |
| Include cells with no regional assignment (NA) | Retain cells whose region label is missing — required to keep `C185-67 Pnoc.Mixed.GABA-2` and other NA-only clusters; off would drop the highest-Pnoc S1 cluster | ON |
| Min POA cells per cluster | Clusters with fewer than this many POA cells drop out of the analysis | 20 |
| Filter by HypoMap detectability | *(Signature refinement)* Drop candidate signature genes that are essentially undetectable in the atlas (cell detection rate below the floor, OR max per-cluster mean log-norm expression below the floor) before π-score ranking / top-N selection | ON |
| Min cell detection rate | Detectability floor: minimum fraction of atlas cells with a non-zero count for the gene (slider step 0.005) | 0.02 |
...
| Filter for cluster specificity | *(Signature refinement)* Drop signature genes expressed broadly across clusters (per-cluster mean log-norm expression above the threshold below in more than the fraction below) — operational proxy for "pan-neuronal / housekeeping, not cell-type-specific"; catches *Sst, Polr2h, Eid2, Pdxp, Ppil1, Arl6ip4, Mrpl12, Emc9, …* | ON |
...
| Tab | Contents |
|---|---|
| **Data Overview** | Gene/cell/cluster counts, match rate, enriched gene list, **signature refinement diagnostics** (per-gene drop log), **POA restriction diagnostics** (when active), bacTRAP volcano plot, gene matching diagnostics |
...
- **Filter A — HypoMap detectability.** Drop genes essentially absent from the atlas — fraction of cells expressing the gene below the *Min cell detection rate* floor (default 0.02), OR maximum per-cluster mean log-norm expression below the *Min max-cluster-mean expression* floor (default 0.05). A gene that never lands in any cell's top-τ window cannot contribute to AUCell and only adds noise.
- **Filter B — Cluster specificity.** Drop genes expressed broadly across clusters — per-cluster mean log-norm expression above the *Specificity: cluster mean threshold* (default 0.5) in more than the *Specificity: max cluster fraction* (default 0.5) of clusters. Housekeeping / translational-machinery / pan-neuronal genes (*Sst, Polr2h, Eid2, Pdxp, Ppil1, Arl6ip4, Mrpl12, Emc9, Snap25, …*) are enriched in any ribosomally-active neuron but carry no cell-type specificity; "expressed broadly across clusters" is a clean operational proxy that needs no curated list.

Filters run in order (detectability first, then specificity); a gene that would fail both is reported against whichever caught it first. The per-cluster mean expression (per matched gene) and the per-gene atlas detection rate are cached on `adata.uns` (keyed by the cluster column, and — when an atlas restriction is active — by its mask signature) so they're computed once per atlas load. The per-gene **drop log** — `gene, status` (`kept` / `dropped_detectability` / `dropped_specificity` / `dropped_not_in_atlas`)`, detection_rate, max_cluster_mean, frac_clusters_above_thresh, reason` — appears in the **Signature refinement diagnostics** expander on the Data Overview tab (one-line summary, the full sortable table, plus a "Top 20 dropped genes by detection rate" view that surfaces the broadly-expressed contaminants) and is downloadable as `signature_refinement_droplog_{filter_signature}.csv`. A tripwire logs a `WARNING` if *Pnoc* is dropped (sanity check only — the filter still completes). When at least one refinement filter is active, filtered CSV exports gain a `_refined` segment in their filter-signature suffix. Disable either filter from the sidebar to return to the pre-change candidate set.

### 11. POA-only Atlas Restriction (default OFF)

A single sidebar toggle (*Restrict to POA cells only*, in the "Atlas restriction" expander) reruns the **entire** pipeline against a preoptic-area subset of the atlas, so the analysis can be reported in two parallel modes: "what does the Cre line capture across the whole hypothalamus" (default) and "what does it capture within POA, directly comparable to HypoMap Table S1" (this toggle). This is a **cell-level** restriction (matching S1's "per-cluster Pnoc means computed within POA cells"), not a cluster-level one. A cell is POA-compatible iff its `Region_summarized` value is missing (when *Include cells with no regional assignment* is on, the default — this retains the highest-Pnoc S1 cluster `C185-67 Pnoc.Mixed.GABA-2`, which is 100 % NA) or any POA keyword (case-insensitive substring, default `preoptic`, configurable as a comma-separated list) appears in its `Region_summarized` value. When the toggle is on:

- A `adata_view = adata[poa_mask].copy()` is built once and cached in session state; the original atlas is left untouched in `st.cache_resource`. Every downstream computation — cluster means, cell detection rate, gene means for the empirical-null quantile binning, marker genes, AUCell per-cell scoring, the empirical null, the Cre-driver baseline filter, and the four orthogonal methods — runs on `adata_view`.
- All cached atlas statistics on `adata.uns` carry a mask signature (`poaonly_<sorted-keywords>[_na]`) so toggling never reuses full-atlas data.
- `compute_marker_genes` is recomputed on `adata_view` (the precomputed markers shipped in `.uns` reflect full-atlas specificity and would give wrong Fisher/GSEA results); a spinner is shown — on a ~22 k-cell POA subset Wilcoxon takes 1–3 min vs ~15 min on the full atlas.
- The empirical null's expression-matched control sets are sampled from POA-restricted expression quantile bins and scored on `adata_view`, so z-scores stay comparable across modes.
- The Cre-driver baseline cutoff now compares against POA-restricted Pnoc values (a different scale — POA-restricted values are larger for POA-enriched genes, so the same numeric cutoff is stricter; the slider tooltip reflects the mode, and `0.03` is the recommended Pnoc-Cre value when the warm-sensitive minority should be kept).
- Clusters with fewer than *Min POA cells per cluster* POA cells drop out; the "POA restriction diagnostics" expander on the Data Overview tab reports cell counts, the top retained/excluded regions, and which clusters dropped out (with their full-atlas counts).
- Filtered CSV exports gain a `_poaonly_<keywords>[_na]` segment (collapsed to `_poaonly` if more than two custom keywords).

Off by default; when off, behaviour is identical to before this change.
...
2. **Signature refinement filters** (default ON). The candidate signature is filtered for HypoMap detectability and cluster specificity before π-score ranking, so the top-N signature — and therefore AUCell and every other downstream method — may include different genes than before. A per-gene drop log is surfaced in the Data Overview tab and exported as `signature_refinement_droplog_{filter_signature}.csv`; filtered CSV exports gain a `_refined` segment in their filename when a refinement filter is active.
3. **POA-only atlas restriction** (default OFF, opt-in). A new optional analysis variant that reruns the whole pipeline over preoptic-area cells only (Table-S1-comparable); no effect unless enabled. Replaces the earlier neuronal-only atlas mask.

**To reproduce the exact pre-change behaviour:** turn off *Compute empirical-null z-scores*, *Filter by HypoMap detectability* and *Filter for cluster specificity* (leave *Restrict to POA cells only* off). With those three unchecked, the pipeline returns to its earlier outputs bit-for-bit, and CSV filenames carry no `_refined` / `_poaonly…` / `_nullN` suffix. CSV column additions are purely additive — no existing column is renamed or removed.
