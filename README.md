...
**To reproduce the exact pre-change behaviour:** turn off *Compute empirical-null z-scores*, *Filter by HypoMap detectability* and *Filter for cluster specificity* (leave *Restrict to neurons only* off). With those three unchecked, the pipeline returns to its earlier outputs bit-for-bit, and CSV filenames carry no `_refined` / `_neuronal` / `_nullN` suffix. CSV column additions are purely additive — no existing column is renamed or removed.
